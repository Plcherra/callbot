 ...
 *
 * Scaling: With a single server.js process this works. With multiple instances behind
 * a load balancer, the webhook may hit instance A while the WebSocket connects to instance B;
 * B has no cached prompt and falls back to fetchPrompt (HTTP to /api/receptionist-prompt),
 * which adds latency. For production multi-instance deployments, consider a shared cache
 * (e.g. Redis) keyed by call_control_id.
 */
