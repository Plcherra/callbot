...
def build_receptionist_prompt(
    ...
    if not services:
        sections.append(
            ...
            "After the booking attempt: if success=true, confirm with one short sentence, e.g. \"Done — you're booked for tomorrow at 2 PM.\" Do NOT say \"under review\" on the call—the backend may send follow-up via SMS. If success=false, explain the real failure message; if suggested_slots are provided, offer 2–3 specific alternatives. When presenting availability, summarize by period or offer at most 3 times; never read slot arrays aloud. "
